...
loadBrowserConfig();
function hoshiEmbedUrl(target){return HOSHI_URL+'/?go='+encodeURIComponent(target)+'&embed=1';}async function api(type,q){const endpoint=type==='manga'?'/api/search/manga':type==='anime'?'/api/search/anime':'/api/search/characters';try{const r=await fetch(`${apiUrl(endpoint)}?q=${encodeURIComponent(q)}&limit=12`);const d=await r.json().catch(()=>({}));if(!r.ok){console.warn('api',type,d.error||r.status);return []}return d.data||[]}catch(e){console.warn('api fail',type,e);return []}}
...
async function openNotifications(){
 ...
 if(error)return toast(error.message,'error');
 showModal(`<span class="eyebrow">SEKAI</span><h2>Notificaciones</h2><div class="notification-list">${(data||[]).map(n=>`<button class="notification-row ${n.read?'':'unread'}" data-notification="${esc(n.id)}"><b>${esc(n.title)}</b><small>${esc(n.body||'')} · ${new Date(n.created_at).toLocaleDateString('es')}</small></button>`).join('')||'<p class="muted">No tienes notificaciones.</p>'}</div>`);
 $$('.notification-row').forEach(b=>b.onclick=async()=>{await sb.from('notifications').update({read:true}).eq('id',b.dataset.notification).eq('user_id',state.user.id);b.classList.remove('unread');});
}
...
function musicPicker(id='editorMusic',allowNone=true){
  const opts = PRISM_CATALOG.map(m=>`<button type="button" class="music-opt" data-mid="${m.id}" data-target="${id}"><b>${esc(m.title)}</b><small>${esc(m.artist)}</small></button>`).join('');
  return `<div class="music-picker liquid-panel">
    ...
    <div class="music-opt-list" id="${id}_list">
      ${allowNone?`<button type="button" class="music-opt active" data-mid="" data-target="${id}"><b>Sin música</b><small>Opcional</small></button>`:''}
      ${opts}
    </div>
    ...
    <div class="music-note">${PRISM_REPO?'Catálogo listo.':'Configura PRISM_REPO para audio.'}</div>
  </div>`;
}
function bindMusicPickers(root=document){
  root.querySelectorAll('.music-opt').forEach(b=>{
    b.onclick=()=>{
      ...
      if(lyrCb && lyrCb.checked && b.dataset.mid) autoFillLyrics(b.dataset.mid, textId);
    };
  });
...
function openStoryPanel(tool){
  ...
  if(tool==='media'){
    ...
    $('#storyMusicSelect').onchange=e=>{ window._storyState.musicId=e.target.value; $('#storyMusicId').value=e.target.value; };
  } else if(tool==='effect'){
    panel.innerHTML = `
      <div class="ig-panel-inner">
