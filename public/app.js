...
/* Inline SVG icons: no emoji glyphs, consistent across devices. */
function svgIcon(name, label=''){
  const paths={
    close:'<path d="M6 6l12 12M18 6L6 18"/>',
    settings:'<path d="M12 3l1.4 2.8 3.1.5 2.2-2.2 2.1 2.1-2.2 2.2.5 3.1L21 13l-1 2.8-2.8 1.4-.5 3.1-2.2 2.2-2.1-2.1-2.2 2.1-2.2-2.2-.5-3.1L4 15.8 3 13l2.8-1.4.5-3.1L4.1 6.3l2.1-2.1 2.2 2.2 3.1-.5z"/><circle cx="12" cy="12" r="3"/>',
    camera:'<rect x="3" y="6" width="18" height="14" rx="3"/><path d="M8 6l1.5-3h5L16 6"/><circle cx="12" cy="13" r="4"/>',
    sticker:'<rect x="3" y="3" width="18" height="18" rx="4"/><path d="M8 8h.01M16 8h.01M8 15c2.5 2 5.5 2 8 0"/>',
    music:'<path d="M9 18V5l10-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="16" cy="16" r="3"/>',
    sparkle:'<path d="M12 2l2.2 7.8L22 12l-7.8 2.2L12 22l-2.2-7.8L2 12l7.8-2.2z"/>',
    image:'<rect x="3" y="4" width="18" height="16" rx="2"/><circle cx="8" cy="9" r="1.5"/><path d="M3 16l5-5 4 4 3-3 6 6"/>',
    video:'<rect x="3" y="5" width="18" height="14" rx="2"/><path d="M10 9l5 3-5 3z"/>',
    palette:'<path d="M12 3a9 9 0 0 0 0 18h2a2 2 0 0 0 0-4h-1a2 2 0 0 1 0-4h3a5 5 0 0 0 0-10z"/><circle cx="7" cy="10" r="1"/><circle cx="10" cy="7" r="1"/><circle cx="15" cy="7" r="1"/>',
    ...
    home:'<path d="M4 10l8-7 8 7v9a1 1 0 0 1-1 1h-5v-6H10v6H5a1 1 0 0 1-1-1z"/>',
    ...
    at:'<circle cx="12" cy="12" r="8"/><path d="M16 12a4 4 0 1 1-1.2-2.8"/><path d="M16 8v5.5a1.5 1.5 0 0 0 3 0"/>',
    poll:'<path d="M5 20V10M12 20V4M19 20v-7"/>',
    hash:'<path d="M8 4l-2 16M18 4l-2 16M4 9h16M3 15h16"/>',
    question:'<circle cx="12" cy="12" r="9"/><path d="M9.5 9a2.5 2.5 0 1 1 3.5 2.3c-.8.4-1.5 1-1.5 2.2"/><circle cx="12" cy="17" r=".8"/>',
    gif:'<rect x="3" y="6" width="18" height="12" rx="2"/><path d="M7 15v-4.5a1.5 1.5 0 0 1 3 0V15M7 12h2M13 10.5v4h2.5"/>'
  };
  const body=paths[name]||paths.sparkle;
  return `<svg class="svg-icon" viewBox="0 0 24 24" role="img" aria-label="${label}" focusable="false">${body}</svg>`;
}
...
const PRISM_API_KEY=String(window.SEKAI_MUSIC_API_KEY||window.PRISM_API_KEY||'').trim();
let prismCatalogPromise=null;

function prismFetchHeaders(){
  const h={ cache:'no-store' };
  if(PRISM_API_KEY){
    return { cache:'no-store', headers:{ 'x-api-key': PRISM_API_KEY, 'Accept':'application/json' } };
  }
  return { cache:'no-store' };
}
function prismUrl(pathOrUrl){
  if(!pathOrUrl) return '';
...
function prismAuthedUrl(url){
  if(!PRISM_API_KEY||!url) return url;
  try{
    const u=new URL(url, PRISM_REPO||undefined);
    if(!u.searchParams.has('api_key')) u.searchParams.set('api_key', PRISM_API_KEY);
    return u.toString();
  }catch{ return url; }
}
...
async function loadPrismCatalogFromRepo(){
  ...
  for (const baseUrl of urls) {
    try {
      const url = prismAuthedUrl(baseUrl);
      const r = await fetch(url, prismFetchHeaders());
      if (!r.ok) continue;
      ...
      console.log('PRISM catalog loaded', PRISM_CATALOG.length, 'from', baseUrl);
      return;
    } catch (e) { console.warn('PRISM catalog fail', baseUrl, e); }
  }
...
async function fetchLyrics(artist, title){
  ...
  try{
    const base = PRISM_REPO + '/api/lyrics?artist=' + encodeURIComponent(artist||'') + '&title=' + encodeURIComponent(title);
    const u = prismAuthedUrl(base);
    const r = await fetch(u, prismFetchHeaders());
    if(!r.ok) return null;
...
function searchMusic(q){const n=q.toLowerCase();return PRISM_CATALOG.filter(x=>(x.title+' '+x.artist).toLowerCase().includes(n)).slice(0,24)}
function playMusicFull(idOrTrack){
  const m=typeof idOrTrack==='object'?idOrTrack:resolvePrismTrack(idOrTrack);
  if(!m){toast('Pista no encontrada','error');return;}
  const list=PRISM_CATALOG.length?PRISM_CATALOG:[m];
  showModal(`<div class="music-player-sheet">
    <span class="eyebrow">SEKAI MUSIC</span>
    <div class="mp-cover-wrap">${m.cover?`<img class="mp-cover" src="${esc(m.cover)}" alt="">`:`<div class="mp-cover ph">${svgIcon('music','Música')}</div>`}</div>
    <h2 class="mp-title">${esc(m.title)}</h2>
    <p class="mp-artist">${esc(m.artist||'')}</p>
    <audio id="mpAudio" controls autoplay src="${esc(musicUrl(m))}" style="width:100%;margin:12px 0"></audio>
    <div class="mp-list">${list.map(t=>`<button type="button" class="mp-track ${String(t.id)===String(m.id)?'on':''}" data-mid="${esc(t.id)}"><span>${esc(t.title)}</span><small>${esc(t.artist||'')}</small></button>`).join('')}</div>
  </div>`);
  $$('.mp-track').forEach(b=>b.onclick=()=>{
    const t=resolvePrismTrack(b.dataset.mid); if(!t)return;
    const a=$('#mpAudio'); if(a){a.src=musicUrl(t);a.play().catch(()=>{});}
    $$('.mp-track').forEach(x=>x.classList.toggle('on',x===b));
    const title=$('.mp-title'), artist=$('.mp-artist'), cover=$('.mp-cover');
    if(title)title.textContent=t.title; if(artist)artist.textContent=t.artist||'';
    if(cover&&t.cover){if(cover.tagName==='IMG')cover.src=t.cover;}
  });
}
function openMusicBrowser(){
  if(!PRISM_CATALOG.length){toast('Cargando catálogo…'); loadPrismCatalogFromRepo().then(()=>openMusicBrowser()); return;}
  playMusicFull(PRISM_CATALOG[0]);
}
...
function normalizeGlass(v){
  ...
  // DB may be integer 55-96 or legacy float 0.55-0.96
  if(n>0 && n<=1) n=Math.round(n*100);
  return Math.max(55, Math.min(96, Math.round(n)));
}
function glassAlpha(v){ return normalizeGlass(v)/100; }
function applyCustomization(p={}){
  const root=document.documentElement;
  const saved=JSON.parse(localStorage.getItem('sekai_customization')||'{}');
  const c={...saved,...p};
  if(c.theme_color){root.style.setProperty('--theme',c.theme_color);root.style.setProperty('--purple',c.theme_color);}
  if(c.accent_color){root.style.setProperty('--accent',c.accent_color);root.style.setProperty('--purple2',c.accent_color);}
  root.style.setProperty('--glass-alpha',String(glassAlpha(c.glass_intensity??86)));
  document.body.dataset.motion=c.animation_level||'full';
  document.body.dataset.nav=c.navigation_style||'normal';
  document.body.dataset.bg=c.background_mode||'soft';
  if(c.profile_bg) document.body.style.setProperty('--profile-bg', c.profile_bg);
  if(c.chat_bg) document.body.style.setProperty('--chat-bg', c.chat_bg);
  if(c.font_family) document.body.style.setProperty('--ui-font', c.font_family);
  document.body.classList.toggle('contrast-high',!!c.high_contrast);
  document.body.classList.toggle('ui-dense',!!c.dense_ui);
  document.body.classList.toggle('font-large',!!c.large_font||localStorage.getItem('sekai_large_font')==='1');
  document.body.classList.toggle('cinema-mode',localStorage.getItem('sekai_cinema')==='1');
}
...
/* ========== Giphy API (principal + backup) — chats, stories, posts, shorts, video, perfil ========== */
const GIPHY_KEYS = [
  'PH4rbBOCP2JAb5MLrEz3ErgnUy1ZKlDd', // principal
  'y1UYimEs46Hujw3Tnod2dj8zYO8fkKoq'  // backup
];
let _giphyKeyIndex = 0;

async function giphyFetch(endpoint, params = {}) {
  const qs = new URLSearchParams({ rating: 'pg-13', lang: 'es', limit: String(params.limit || 24), ...params });
  let lastErr = null;
  for (let attempt = 0; attempt < GIPHY_KEYS.length; attempt++) {
    const key = GIPHY_KEYS[(_giphyKeyIndex + attempt) % GIPHY_KEYS.length];
    qs.set('api_key', key);
    try {
      const r = await fetch(`https://api.giphy.com/v1/gifs/${endpoint}?${qs}`, { cache: 'no-store' });
      if (r.status === 429 || r.status === 403) { lastErr = new Error('Giphy limit'); continue; }
      if (!r.ok) throw new Error('Giphy ' + r.status);
      const data = await r.json();
      _giphyKeyIndex = (_giphyKeyIndex + attempt) % GIPHY_KEYS.length;
      return data;
    } catch (e) { lastErr = e; }
  }
  throw lastErr || new Error('Giphy no disponible');
}

async function searchGiphy(q, limit = 24) {
  if (!q || !String(q).trim()) return giphyFetch('trending', { limit });
  return giphyFetch('search', { q: String(q).trim(), limit });
}

/** Picker universal. onSelect(url, meta). hostEl opcional (panel interno); si no, modal flotante. */
function openGiphyPicker(onSelect, opts = {}) {
  const title = opts.title || 'Buscar GIF';
  const host = opts.hostEl || null;

  const bodyHtml = `
    <div class="giphy-panel">
      <div class="giphy-search-row">
        <input type="search" class="giphy-input" data-giphy-q placeholder="Buscar GIFs…" autocomplete="off">
        <button type="button" class="giphy-btn" data-giphy-go>Buscar</button>
      </div>
      <div class="giphy-grid" data-giphy-grid><div class="giphy-loading">Cargando GIFs…</div></div>
      <p class="giphy-credit">Powered by GIPHY</p>
    </div>`;

  let root;
  if (host) {
    host.hidden = false;
    host.innerHTML = `<div class="ig-panel-inner">${bodyHtml}</div>`;
    root = host;
  } else {
    // modal overlay ligero encima de todo
    let ov = document.getElementById('giphyOverlay');
    if (!ov) {
      ov = document.createElement('div');
      ov.id = 'giphyOverlay';
      ov.className = 'giphy-overlay';
      document.body.appendChild(ov);
    }
    ov.innerHTML = `
      <div class="giphy-sheet" role="dialog" aria-label="${esc(title)}">
        <header class="giphy-sheet-head">
          <strong>${esc(title)}</strong>
          <button type="button" class="giphy-close" data-giphy-close aria-label="Cerrar">×</button>
        </header>
        ${bodyHtml}
      </div>`;
    ov.hidden = false;
    root = ov;
    ov.querySelector('[data-giphy-close]').onclick = () => { ov.hidden = true; };
    ov.onclick = (e) => { if (e.target === ov) ov.hidden = true; };
  }

  const input = root.querySelector('[data-giphy-q]');
  const grid = root.querySelector('[data-giphy-grid]');
  const goBtn = root.querySelector('[data-giphy-go]');

  const pick = (url, meta) => {
    if (typeof onSelect === 'function') onSelect(url, meta);
    const ov = document.getElementById('giphyOverlay');
    if (ov && !host) ov.hidden = true;
  };

  const run = async (q) => {
    grid.innerHTML = '<div class="giphy-loading">Cargando…</div>';
    try {
      const data = await searchGiphy(q);
      const list = data?.data || [];
      if (!list.length) { grid.innerHTML = '<div class="giphy-empty">Sin resultados</div>'; return; }
      grid.innerHTML = list.map(g => {
        const still = g.images?.fixed_width_small_still?.url || g.images?.preview_gif?.url || '';
        const anim = g.images?.fixed_width?.url || g.images?.downsized?.url || still;
        const full = g.images?.original?.url || g.images?.downsized_medium?.url || anim;
        const title = (g.title || 'GIF').replace(/"/g, '');
        return `<button type="button" class="giphy-item" data-full="${esc(full)}" data-anim="${esc(anim)}" title="${esc(title)}">
          <img src="${esc(still || anim)}" alt="${esc(title)}" loading="lazy">
        </button>`;
      }).join('');
      grid.querySelectorAll('.giphy-item').forEach(btn => {
        const img = btn.querySelector('img');
        btn.onmouseenter = () => { if (btn.dataset.anim) img.src = btn.dataset.anim; };
        btn.onclick = () => pick(btn.dataset.full || btn.dataset.anim, { title: btn.title });
      });
    } catch (e) {
      console.error(e);
      grid.innerHTML = '<div class="giphy-empty">No se pudieron cargar GIFs</div>';
      toast('Giphy no respondió','error');
    }
  };
...
function addStoryGif(url) {
  if (!url) return;
  const layer = $('#storyStickersLayer') || $('#storyCanvas');
  if (!layer) return;
  const el = document.createElement('div');
  el.className = 'ig-sticker ig-sticker-gif studio-layer';
  el.dataset.layer = 'gif';
  el.innerHTML = `<img src="${esc(url)}" alt="GIF" draggable="false">`;
  layer.appendChild(el);
  makeStudioLayer(el, { kind: 'gif', x: 20 + Math.random() * 40, y: 20 + Math.random() * 35 });
  if (window._storyState) {
    window._storyState.stickers = window._storyState.stickers || [];
    window._storyState.stickers.push({ type: 'gif', url });
  }
}


/* ========== Custom selects (no native browser pickers) ========== */
function sekaiSelect(id, options, value, opts={}) {
  const label = opts.label || '';
  const current = options.find(o => String(o.value) === String(value)) || options[0] || {value:'',label:'—'};
  const optsHtml = options.map(o =>
    `<button type="button" class="sk-opt ${String(o.value)===String(current.value)?'selected':''}" data-value="${esc(o.value)}" role="option">${esc(o.label)}</button>`
  ).join('');
  return `<div class="sk-select" id="${id}_wrap" data-select-id="${id}">
    ${label?`<span class="sk-select-label">${esc(label)}</span>`:''}
    <button type="button" class="sk-select-trigger" id="${id}_btn" aria-haspopup="listbox" aria-expanded="false">
      <span class="sk-select-value" id="${id}_val">${esc(current.label)}</span>
      <span class="sk-select-chevron" aria-hidden="true">▾</span>
    </button>
    <input type="hidden" id="${id}" value="${esc(current.value)}">
    <div class="sk-select-menu" id="${id}_menu" role="listbox" hidden>${optsHtml}</div>
  </div>`;
}

function bindSekaiSelects(root=document) {
  root.querySelectorAll('.sk-select').forEach(wrap => {
    if (wrap.dataset.bound === '1') return;
    wrap.dataset.bound = '1';
    const id = wrap.dataset.selectId;
    const btn = wrap.querySelector('.sk-select-trigger');
    const menu = wrap.querySelector('.sk-select-menu');
    const hidden = wrap.querySelector('input[type=hidden]');
    const valEl = wrap.querySelector('.sk-select-value');
    ...
    const open = () => {
      // close others
      document.querySelectorAll('.sk-select.open').forEach(w => {
        if (w !== wrap) {
          w.classList.remove('open');
          const m = w.querySelector('.sk-select-menu');
          const b = w.querySelector('.sk-select-trigger');
          if (m) m.hidden = true;
    ...
    menu.querySelectorAll('.sk-opt').forEach(opt => {
      opt.addEventListener('click', e => {
        ...
        menu.querySelectorAll('.sk-opt').forEach(x => x.classList.toggle('selected', x === opt));
        close();
...
// close selects on outside click (once)
if (!window.__sekaiSelectOutside) {
  window.__sekaiSelectOutside = true;
  document.addEventListener('click', e => {
    if (e.target.closest('.sk-select')) return;
    document.querySelectorAll('.sk-select.open').forEach(w => {
      w.classList.remove('open');
      const m = w.querySelector('.sk-select-menu');
      const b = w.querySelector('.sk-select-trigger');
...
async function sendChatGif(url) {
  if (!url || !activeConversation?.userId || !supabase || !state.user) return;
  const rid = activeConversation.userId;
  try {
    const { data, error } = await supabase.from('messages').insert({
      sender_id: state.user.id, recipient_id: rid, body: '[gif]', media_url: url
    }).select().single();
    if (error) {
      const r2 = await supabase.from('messages').insert({
        sender_id: state.user.id, recipient_id: rid, body: '[gif] ' + url
      }).select().single();
    ...
    toast(e.message || 'Error al enviar GIF', 'error');
  }
}
...
<button class="oauth guest-choice" data-onboard="guest"><span>♙</span><b>Continuar como invitado</b><em>›</em></button><small class="legal">Al continuar, aceptas nuestros Términos de servicio y Política de privacidad.</small></div>`;
 ...
 else if(step==='avatar')c.innerHTML=`<div class="ob-page compact-ob"><div class="ob-brandline"><span class="onboard-mark small">✦</span><b>Sekai</b><small>4/7</small></div><span class="eyebrow">TU FOTO DE PERFIL</span><h1>Elige una imagen para representarte.</h1><p>Sube una foto o elige un avatar. Puedes cambiarla después.</p><div class="avatar-preview" id="avatarPreview">${state.selectedAvatarUrl?`<img src="${esc(state.selectedAvatarUrl)}" alt="">`:'U'}<span class="camera-badge">+</span></div><div class="avatar-grid">${['1000059278-Photoroom.png','1000059279-Photoroom.png','1000059280-Photoroom.png','1000059281-Photoroom.png','1000059282-Photoroom.png','1000059283-Photoroom.png','1000059284-Photoroom.png'].map((f,i)=>`<button type="button" class="avatar-choice ${state.selectedAvatarUrl===`assets/${f}`?'selected':''}" data-avatar="assets/${f}"><img src="assets/${f}" alt="Avatar ${i+1}"></button>`).join('')}<button type="button" class="avatar-choice add-avatar" id="customAvatarBtn">+</button></div><input id="profileAvatar" type="file" accept="image/*" hidden><div class="avatar-note">También puedes subir tu propia foto o un GIF.</div><button class="primary wide" id="avatarNext">Siguiente <span>→</span></button><div class="ob-dots"><i></i><i></i><i></i><i class="active"></i><i></i><i></i><i></i></div></div>`;
 else if(step==='bio')c.innerHTML=`<div class="ob-page compact-ob"><div class="ob-brandline"><span class="onboard-mark small">✦</span><b>Sekai</b><small>5/7</small></div><span class="eyebrow">BIO Y ESTADO</span><h1>Cuéntale a Sekai quién eres.</h1><p>Bio corta y un estado que se verá en tu perfil.</p><form id="bioForm" class="form"><label>Bio<textarea id="obBio" maxlength="160" rows="3" placeholder="Me gusta el anime, el manga y explorar…">${esc(draft.bio||'')}</textarea></label><label>Estado / tagline<input id="obStatus" maxlength="48" placeholder="ej. Explorando Sekai" value="${esc(draft.status||'')}"></label><button class="primary wide" type="submit">Siguiente <span>→</span></button></form><div class="ob-dots"><i></i><i></i><i></i><i></i><i class="active"></i><i></i><i></i></div></div>`;
 else if(step==='style')c.innerHTML=`<div class="ob-page compact-ob style-ob"><div class="ob-brandline"><span class="onboard-mark small">✦</span><b>Sekai</b><small>6/7</small></div><span class="eyebrow">PERSONALIZA TU SEKAI</span><h1>Colores, glass y estilo.</h1><p>Vista previa en vivo. Todo se puede cambiar después.</p>
<div class="live-preview-card" id="livePreview">
  <div class="lp-header"><span class="lp-logo">✦ Sekai</span><span class="lp-pill">Preview</span></div>
  <div class="lp-profile">
    <div class="lp-avatar">${state.selectedAvatarUrl?`<img src="${esc(state.selectedAvatarUrl)}" alt="">`:'U'}</div>
    <div><b>${esc(draft.nick||'Usuario')}</b><small>@${esc(draft.username||'user')}</small></div>
  </div>
  <div class="lp-card glass-demo">Tarjeta con glass y acento</div>
  <div class="lp-nav"><span></span><span class="on"></span><span></span></div>
</div>
<div class="style-controls">
  <label>Tema <input type="color" id="obTheme" value="${esc(draft.theme||'#7068e8')}"></label>
  <label>Acento <input type="color" id="obAccent" value="${esc(draft.accent||'#a29cf5')}"></label>
  <label>Glass
    <select id="obGlass"><option value="72">Suave</option><option value="86" selected>Medio</option><option value="95">Intenso</option></select>
  </label>
  <label>Fondo
    <select id="obBg"><option value="soft">Suave</option><option value="plain">Limpio</option><option value="night">Noche</option></select>
  </label>
  <label>Fuente
    <select id="obFont"><option value="system-ui">Sistema</option><option value="'Plus Jakarta Sans',system-ui">Jakarta</option><option value="Georgia,serif">Serif</option><option value="'Courier New',monospace">Mono</option></select>
  </label>
</div>
<button class="primary wide" id="styleNext">Crear perfil <span>→</span></button>
<div class="ob-dots"><i></i><i></i><i></i><i></i><i></i><i class="active"></i><i></i></div></div>`;
 else if(step==='welcome')c.innerHTML=`<div class="ob-page welcome-final cinematic-card"><div class="welcome-art"></div><span class="eyebrow">7/7 · SEKAI LISTO</span><h1>Bienvenido a Sekai</h1><p>Tu aventura comienza ahora.</p><div class="welcome-id"><span>▣</span><div><small>ID DE SEKAI</small><b>${esc(state.profile?.chromi_id||'')}</b><small>Guarda esto, es único y te identifica en la comunidad.</small></div></div><button class="primary wide" id="enterSekai">Comenzar</button><div class="ob-dots"><i></i><i></i><i></i><i></i><i class="active"></i></div></div>`;
 ...
 $('#customAvatarBtn')?.addEventListener('click',()=>$('#profileAvatar').click());$('#profileAvatar')?.addEventListener('change',e=>{const f=e.target.files?.[0];if(!f)return;state.selectedAvatarUrl=URL.createObjectURL(f);$('#avatarPreview').innerHTML=`<img src="${esc(state.selectedAvatarUrl)}" alt=""> <span class="camera-badge">⌾</span>`});
  if(!document.getElementById('onboardGifAvatar') && $('#customAvatarBtn')){
    const g=document.createElement('button'); g.type='button'; g.id='onboardGifAvatar'; g.className='avatar-choice gif-avatar-btn'; g.textContent='GIF';
    $('#customAvatarBtn').parentElement?.appendChild(g);
    g.onclick=()=>openGiphyPicker(url=>{
      state.selectedAvatarUrl=url;
      $$('[data-avatar]').forEach(x=>x.classList.remove('selected'));
      $('#avatarPreview').innerHTML=`<img src="${esc(url)}" alt=""> <span class="camera-badge">⌾</span>`;
      toast('GIF de perfil elegido');
    }, { title: 'GIF de perfil' });
  }
 ...
 $('#avatarNext')?.addEventListener('click',()=>{
   ...
   renderOnboarding('bio');
 });
 $('#bioForm')?.addEventListener('submit',e=>{
   e.preventDefault();
   const draft=state.onboardDraft||{};
   draft.bio=$('#obBio')?.value?.trim()||'';
   draft.status=$('#obStatus')?.value?.trim()||'';
   state.onboardDraft=draft;
   renderOnboarding('style');
 });
 function bindStyleLive(){
   const applyLive=()=>{
     const theme=$('#obTheme')?.value||'#7068e8';
     const accent=$('#obAccent')?.value||'#a29cf5';
     const glass=normalizeGlass($('#obGlass')?.value||86);
     const bg=$('#obBg')?.value||'soft';
     const font=$('#obFont')?.value||'system-ui';
     const root=document.documentElement;
     root.style.setProperty('--theme',theme);
     root.style.setProperty('--purple',theme);
     root.style.setProperty('--accent',accent);
     root.style.setProperty('--glass-alpha',String(glass/100));
     document.body.dataset.bg=bg;
     document.body.style.setProperty('--ui-font',font);
     const lp=$('#livePreview');
     if(lp){ lp.style.setProperty('--theme',theme); lp.style.setProperty('--accent',accent); }
     const draft=state.onboardDraft||{};
     draft.theme=theme; draft.accent=accent; draft.glass=glass; draft.bg=bg; draft.font=font;
     state.onboardDraft=draft;
   };
   ['obTheme','obAccent','obGlass','obBg','obFont'].forEach(id=>{
     const el=$('#'+id); if(el) el.addEventListener('input',applyLive);
     if(el) el.addEventListener('change',applyLive);
   });
   applyLive();
 }
 bindStyleLive();
 $('#styleNext')?.addEventListener('click',async()=>{
   if(!state.user)return toast('Tu sesión no está disponible.','error');
   const draft=state.onboardDraft||{};
   ...
   const bio=draft.bio||'', visibility=draft.visibility||'public', status_text=draft.status||'';
   let avatar_url=state.profile?.avatar_url||null;
   const file=$('#profileAvatar')?.files?.[0];
   if(file){
     const ext=(file.name.split('.').pop()||'jpg').toLowerCase(),path=`${state.user.id}/avatar.${ext}`;
     const up=await supabase.storage.from('avatars').upload(path,file,{upsert:true,contentType:file.type});
     if(up.error)return toast('No pude subir la foto: '+up.error.message,'error');
     avatar_url=supabase.storage.from('avatars').getPublicUrl(path).data.publicUrl;
   } else if(state.selectedAvatarUrl && !String(state.selectedAvatarUrl).startsWith('blob:')){
     avatar_url=state.selectedAvatarUrl;
   } else if(state.selectedAvatarUrl && String(state.selectedAvatarUrl).startsWith('blob:')){
     try{
       const blob=await fetch(state.selectedAvatarUrl).then(r=>r.blob());
       const path=`${state.user.id}/avatar-${Date.now()}.jpg`;
       const up=await supabase.storage.from('avatars').upload(path,blob,{upsert:true,contentType:blob.type||'image/jpeg'});
   ...
   const payload={
     id:state.user.id,username,display_name:nick,bio,visibility,avatar_url,status_text,chromi_id,
     theme_color:draft.theme||'#7068e8', accent_color:draft.accent||'#a29cf5',
     glass_intensity:normalizeGlass(draft.glass||86), background_mode:draft.bg||'soft'
   };
   const {data,error}=await supabase.from('profiles').upsert(payload,{onConflict:'id'}).select().single();
   if(error){if(error.code==='23505')return toast('Ese nombre de usuario ya está ocupado.','error');return toast(error.message,'error')}
   state.profile=data; state.onboardDraft=null; localStorage.removeItem('chromi_onboarding_pending');
   localStorage.setItem('sekai_customization', JSON.stringify({theme_color:payload.theme_color,accent_color:payload.accent_color,glass_intensity:payload.glass_intensity,background_mode:payload.background_mode,font_family:draft.font}));
   applyCustomization(data);
   if(draft.font) document.body.style.setProperty('--ui-font', draft.font);
   $('#topName').textContent=data.display_name||data.username;
   $('#homeProfileText').textContent='@'+data.username+' · '+data.chromi_id;
...
function renderFullProfile(){
 const pbg=localStorage.getItem('sekai_profile_bg'); if(pbg) document.body.style.setProperty('--profile-bg',pbg);
 const font=localStorage.getItem('sekai_font'); if(font) document.body.style.setProperty('--ui-font',font);
...
async function openEditProfile(){
 ...
 <form id="editProfileForm" class="form profile-edit-form">
  <div class="st-panel active" data-panel="profile">
   ...
   <div class="media-upload-block">
     ...
     <div class="media-upload-row">
       <div class="media-preview-circle" id="epAvatarPrev">${p.avatar_url?`<img src="${esc(p.avatar_url)}" alt="">`:'<span>U</span>'}</div>
       <div class="media-upload-actions">
         <button type="button" class="secondary" id="epAvatarFileBtn">Subir foto</button>
         <button type="button" class="gif-pick-btn" id="epAvatarGif">GIF</button>
         <input type="file" id="epAvatarFile" accept="image/*" hidden>
         <input type="hidden" id="epAvatar" value="${esc(p.avatar_url||'')}">
       </div>
     </div>
   </div>
   <div class="media-upload-block">
     <span class="eyebrow">BANNER</span>
     <div class="media-upload-row banner">
       <div class="media-preview-banner" id="epBannerPrev">${p.banner_url?`<img src="${esc(p.banner_url)}" alt="">`:'<span>Banner</span>'}</div>
       <div class="media-upload-actions">
         <button type="button" class="secondary" id="epBannerFileBtn">Subir imagen</button>
         <button type="button" class="gif-pick-btn" id="epBannerGif">GIF</button>
         <input type="file" id="epBannerFile" accept="image/*" hidden>
         <input type="hidden" id="epBanner" value="${esc(p.banner_url||'')}">
       </div>
     </div>
   </div>
  ...
  <div class="st-panel" data-panel="look" hidden>
   ...
   <div class="two-col privacy-selects">
    ${sekaiSelect('epMotion',[
      {value:'full',label:'Completas'},
      {value:'reduced',label:'Reducidas'},
      {value:'off',label:'Desactivadas'}
    ], p.animation_level||'full', {label:'Animaciones'})}
    ${sekaiSelect('epGlass',[
      {value:'72',label:'Suave'},
      {value:'86',label:'Medio'},
      {value:'95',label:'Intenso'}
    ], String(Math.round(Number(p.glass_intensity)>1?Number(p.glass_intensity):Number(p.glass_intensity??0.86)*100)||86), {label:'Glass'})}
   </div>
   <div class="two-col privacy-selects">
    ${sekaiSelect('epBg',[
      {value:'soft',label:'Suave'},
      {value:'plain',label:'Limpio'},
      {value:'night',label:'Noche'}
    ], p.background_mode||'soft', {label:'Fondo'})}
    ${sekaiSelect('epNav',[
      {value:'normal',label:'Normal'},
      {value:'compact',label:'Compacta'}
    ], p.navigation_style||'normal', {label:'Navegación'})}
   </div>
  ...
  <div class="st-panel" data-panel="privacy" hidden>
   ...
   <div class="privacy-selects">
    ${sekaiSelect('epMsgPriv',[
      {value:'everyone',label:'Todos'},
      {value:'friends',label:'Solo amigos'},
      {value:'nobody',label:'Nadie'}
    ], mv, {label:'Quién puede enviarte mensajes'})}
    ${sekaiSelect('epFriendPriv',[
      {value:'everyone',label:'Todos'},
      {value:'friends_of_friends',label:'Amigos de amigos'},
      {value:'nobody',label:'Nadie'}
    ], fv, {label:'Quién puede enviarte solicitud de amistad'})}
   </div>
 ...
 bindSekaiSelects(document.getElementById('editProfileForm')||document);
 ...
 $$('[data-preset]').forEach(b=>b.onclick=()=>{
  ...
  applyCustomization({theme_color:pr.theme,accent_color:pr.accent,background_mode:pr.bg,animation_level:$('#epMotion').value,glass_intensity:normalizeGlass($('#epGlass').value),navigation_style:$('#epNav').value});
 });
 ...
 ['epTheme','epAccent','epMotion','epGlass','epBg','epNav'].forEach(id=>{
  $('#'+id)?.addEventListener('change',()=>{
   applyCustomization({
    ...
    glass_intensity:normalizeGlass($('#epGlass').value),
 ...
 async function uploadProfileMedia(file, kind){
  const sb=requireSupabase(); if(!sb||!state.user||!file) return null;
  toast('Subiendo…');
  ...
  if(up.error){
    // try sekai-media bucket
    const up2=await sb.storage.from('sekai-media').upload(path,file,{upsert:true,contentType:file.type});
    if(up2.error) throw up2.error;
    return sb.storage.from('sekai-media').getPublicUrl(path).data.publicUrl;
  }
  return sb.storage.from(bucket).getPublicUrl(path).data.publicUrl;
 }
 $('#epAvatarFileBtn')?.addEventListener('click',()=>$('#epAvatarFile')?.click());
 $('#epBannerFileBtn')?.addEventListener('click',()=>$('#epBannerFile')?.click());
 $('#epAvatarFile')?.addEventListener('change',async e=>{
  const f=e.target.files?.[0]; if(!f)return;
  try{
    const url=await uploadProfileMedia(f,'avatar');
    if(!url)return;
    $('#epAvatar').value=url;
    $('#epAvatarPrev').innerHTML=`<img src="${esc(url)}" alt="">`;
    toast('Avatar listo (guarda para aplicar)');
  }catch(err){toast(err.message||'Error al subir','error');}
 });
 $('#epBannerFile')?.addEventListener('change',async e=>{
  const f=e.target.files?.[0]; if(!f)return;
  try{
    const url=await uploadProfileMedia(f,'banner');
    if(!url)return;
    $('#epBanner').value=url;
    $('#epBannerPrev').innerHTML=`<img src="${esc(url)}" alt="">`;
    toast('Banner listo (guarda para aplicar)');
  }catch(err){toast(err.message||'Error al subir','error');}
 });
 $('#epAvatarGif')?.addEventListener('click',()=>{
  openGiphyPicker(url=>{
    const inp=$('#epAvatar'); if(inp) inp.value=url;
    const prev=$('#epAvatarPrev'); if(prev) prev.innerHTML=`<img src="${esc(url)}" alt="">`;
    toast('GIF de avatar aplicado');
  }, { title: 'GIF de avatar' });
 });
 $('#epBannerGif')?.addEventListener('click',()=>{
  openGiphyPicker(url=>{
    const inp=$('#epBanner'); if(inp) inp.value=url;
    const prev=$('#epBannerPrev'); if(prev) prev.innerHTML=`<img src="${esc(url)}" alt="">`;
    toast('GIF de banner aplicado');
  }, { title: 'GIF de banner' });
 });
...
loadBrowserConfig();
function hoshiEmbedUrl(target){return HOSHI_URL+'/?go='+encodeURIComponent(target)+'&embed=1';}

/** Client fallbacks when backend/Jikan are down */
async function clientSearchAnime(q, limit=12){
  try{
    const query=`query($s:String,$n:Int){Page(perPage:$n){media(search:$s,type:ANIME,sort:SEARCH_MATCH){id title{romaji english native}coverImage{large}description(asHtml:false)averageScore episodes status startDate{year}siteUrl}}}`;
    const r=await fetch('https://graphql.anilist.co',{method:'POST',headers:{'Content-Type':'application/json','Accept':'application/json'},body:JSON.stringify({query,variables:{s:q,n:limit}})});
    if(!r.ok) throw new Error('anilist '+r.status);
    const j=await r.json();
    return (j?.data?.Page?.media||[]).map(m=>({
      type:'anime', source:'AniList', badge:'Anime', id:m.id,
      title:m.title?.romaji||m.title?.english||'Anime',
      titleEnglish:m.title?.english||'', titleJapanese:m.title?.native||'',
      cover:m.coverImage?.large||'', description:(m.description||'').replace(/<[^>]+>/g,' ').slice(0,280),
      score:m.averageScore? (m.averageScore/10).toFixed(1):null, episodes:m.episodes, status:m.status,
      year:m.startDate?.year||null, url:m.siteUrl||`https://anilist.co/anime/${m.id}`
    }));
  }catch(e){ console.warn('client anime',e); return []; }
}
async function clientSearchManga(q, limit=12){
  try{
    const r=await fetch(`https://api.mangadex.org/manga?title=${encodeURIComponent(q)}&limit=${limit}&includes[]=cover_art&order[relevance]=desc`,{headers:{'Accept':'application/json'}});
    if(!r.ok) throw new Error('mangadex '+r.status);
    const j=await r.json();
    return (j?.data||[]).map(m=>{
      const title=m.attributes?.title?.en||Object.values(m.attributes?.title||{})[0]||'Manga';
      const desc=m.attributes?.description?.en||Object.values(m.attributes?.description||{})[0]||'';
      const coverRel=(m.relationships||[]).find(x=>x.type==='cover_art');
      const file=coverRel?.attributes?.fileName;
      const cover=file?`https://uploads.mangadex.org/covers/${m.id}/${file}.256.jpg`:'';
      return {type:'manga',source:'MangaDex',badge:'Manga',id:m.id,title,cover,description:String(desc).slice(0,280),
        status:m.attributes?.status,year:m.attributes?.year,url:`https://mangadex.org/title/${m.id}`};
    });
  }catch(e){ console.warn('client manga',e); return []; }
}

async function api(type,q){
  const endpoint=type==='manga'?'/api/search/manga':type==='anime'?'/api/search/anime':'/api/search/characters';
  try{
    const ctrl=AbortSignal.timeout?AbortSignal.timeout(8000):undefined;
    const r=await fetch(`${apiUrl(endpoint)}?q=${encodeURIComponent(q)}&limit=12`,{signal:ctrl});
    const d=await r.json().catch(()=>({}));
    if(r.ok && Array.isArray(d.data) && d.data.length) return d.data;
    console.warn('api empty/fail',type,d.error||r.status);
  }catch(e){ console.warn('api fail',type,e); }
  // Fallback client-side
  if(type==='anime') return clientSearchAnime(q);
  if(type==='manga') return clientSearchManga(q);
  return [];
}
...
async function runSearch(q,add=true){
 ...
 if(apiBaseMissing()){
  ...
  if(localMusic.length){
   ...
   $$('.music-result').forEach(b=>b.onclick=()=>playMusicFull(b.dataset.musicId));
...
function showCreatePost(){
 ...
 <button class="studio-card" data-editor="short"><b>${svgIcon('video','Short')}</b><strong>Short</strong><small>Reels · video vertical corto</small></button>
...
/* ========== STUDIO LAYERS: drag / resize / select ========== */
function makeStudioLayer(el, opts={}){
  if(!el) return null;
  el.classList.add('studio-layer');
  el.dataset.layer = opts.kind || 'object';
  if(!el.style.left) el.style.left = (opts.x ?? 20) + '%';
  if(!el.style.top) el.style.top = (opts.y ?? 30) + '%';
  el.style.position = 'absolute';
  el.style.touchAction = 'none';
  el.style.userSelect = 'none';
  el.style.cursor = 'grab';
  if(opts.w) el.style.width = opts.w + (String(opts.w).endsWith('%')?'':'%');
  // select
  el.addEventListener('pointerdown', e=>{
    if(e.target.closest('.layer-handle')) return;
    selectStudioLayer(el);
  });
  enableDrag(el);
  if(opts.resizable !== false) ensureResizeHandle(el);
  selectStudioLayer(el);
  return el;
}
function selectStudioLayer(el){
  document.querySelectorAll('.studio-layer.selected').forEach(x=>x.classList.remove('selected'));
  if(el) el.classList.add('selected');
  updateLayerInspector(el);
}
if(!window.__studioLayerKeys){ window.__studioLayerKeys=true; document.addEventListener('keydown',e=>{ if(e.key==='Delete'||e.key==='Backspace'){ const s=document.querySelector('.studio-layer.selected'); if(s && !['INPUT','TEXTAREA'].includes(document.activeElement?.tagName)){ e.preventDefault(); s.remove(); updateLayerInspector(null);} } }); }
function ensureResizeHandle(el){
  if(el.querySelector('.layer-handle')) return;
  const h=document.createElement('span');
  h.className='layer-handle';
  ...
  h.addEventListener('pointerdown',e=>{
    ...
    const onMove=ev=>{
      ...
      const nw=Math.max(40, sw+dw);
      const nh=Math.max(24, sh+dh);
      el.style.width=(nw/parent.width*100)+'%';
      // scale font if text
      if(el.classList.contains('studio-text-layer')){
        const base=parseFloat(el.dataset.fontSize||28);
        const scale=nw/Math.max(sw,1);
        el.style.fontSize=Math.max(12, Math.min(72, base*scale))+'px';
      }
    };
    const onUp=()=>{ h.releasePointerCapture(e.pointerId); h.removeEventListener('pointermove',onMove); h.removeEventListener('pointerup',onUp); };
    h.addEventListener('pointermove',onMove);
    h.addEventListener('pointerup',onUp);
  });
}
function enableDrag(el){
  let dragging=false, ox=0, oy=0, parentRect=null;
  el.addEventListener('pointerdown', e=>{
    if(e.target.closest('.layer-handle')) return;
  ...
  el.addEventListener('pointermove', e=>{
    ...
    let x=e.clientX - parentRect.left - ox;
    let y=e.clientY - parentRect.top - oy;
    // clamp
    x=Math.max(0, Math.min(parentRect.width - 20, x));
    y=Math.max(0, Math.min(parentRect.height - 20, y));
    el.style.left=(x/parentRect.width*100)+'%';
    el.style.top=(y/parentRect.height*100)+'%';
    el.style.right='auto';
...
function updateLayerInspector(el){
  const box=document.getElementById('layerInspector');
  if(!box) return;
  if(!el){ box.innerHTML='<p class="muted tiny">Toca un texto, sticker o música para moverlo</p>'; return; }
  const kind=el.dataset.layer||'objeto';
  box.innerHTML=`
    <div class="layer-insp">
      <b>${esc(kind)}</b>
      <div class="layer-insp-row">
        <button type="button" data-lact="front">Traer al frente</button>
        <button type="button" data-lact="back">Enviar atrás</button>
        <button type="button" data-lact="dup">Duplicar</button>
        <button type="button" data-lact="del" class="danger-lite">Eliminar</button>
      </div>
      <label class="tiny">Opacidad <input type="range" id="layerOpacity" min="20" max="100" value="${Math.round((parseFloat(el.style.opacity)||1)*100)}"></label>
      <label class="tiny">Rotación <input type="range" id="layerRot" min="-45" max="45" value="${parseInt(el.dataset.rot||0,10)}"></label>
    </div>`;
  box.querySelector('[data-lact="front"]')?.addEventListener('click',()=>{ el.style.zIndex=String((+el.style.zIndex||10)+1); });
  box.querySelector('[data-lact="back"]')?.addEventListener('click',()=>{ el.style.zIndex=String(Math.max(1,(+el.style.zIndex||10)-1)); });
  box.querySelector('[data-lact="dup"]')?.addEventListener('click',()=>{
    const clone=el.cloneNode(true);
    clone.style.left=(parseFloat(el.style.left||20)+4)+'%';
    clone.style.top=(parseFloat(el.style.top||20)+4)+'%';
    el.parentElement?.appendChild(clone);
    makeStudioLayer(clone,{kind:el.dataset.layer,resizable:true});
  });
  box.querySelector('[data-lact="del"]')?.addEventListener('click',()=>{ el.remove(); updateLayerInspector(null); });
  $('#layerOpacity')?.addEventListener('input',e=>{ el.style.opacity=String(+e.target.value/100); });
  $('#layerRot')?.addEventListener('input',e=>{
    el.dataset.rot=e.target.value;
    el.style.transform=`rotate(${e.target.value}deg)`;
  });
}
function addStudioText(canvas, text, opts={}){
  if(!canvas) return null;
  const el=document.createElement('div');
  el.className='studio-layer studio-text-layer';
  el.dataset.layer='texto';
  el.dataset.fontSize=String(opts.size||28);
  el.textContent=text||'Toca para editar';
  el.style.fontSize=(opts.size||28)+'px';
  ...
  makeStudioLayer(el,{kind:'texto',x:opts.x??15,y:opts.y??40});
  el.addEventListener('dblclick',()=>{
    const t=prompt('Editar texto', el.textContent);
    if(t!=null) el.textContent=t;
  });
  return el;
}
function addStudioMusicBadge(canvas, track){
  if(!canvas||!track) return null;
  // remove previous
  canvas.querySelectorAll('[data-layer="musica"]').forEach(x=>x.remove());
  const el=document.createElement('div');
  el.className='studio-layer studio-music-badge';
  el.dataset.layer='musica';
  el.innerHTML=`<span class="mb-ic">${svgIcon('music','Música')}</span><span class="mb-meta"><b>${esc(track.title||'Música')}</b><small>${esc(track.artist||'')}</small></span>`;
  canvas.appendChild(el);
  makeStudioLayer(el,{kind:'musica',x:8,y:78,resizable:false});
  return el;
}
...
function showStoryEditor(){
  ...
  <div class="ig-story-editor">
    <header class="ig-top">
      <button type="button" class="ig-close" id="storyClose" aria-label="Cerrar">${svgIcon('close','Cerrar')}</button>
      <span class="ig-title">Nueva historia</span>
      <button type="button" class="ig-settings" aria-label="Configuración">${svgIcon('settings','Configuración')}</button>
    </header>
    <div class="ig-canvas" id="storyCanvas">
      <div class="ig-media-layer" id="storyMediaLayer">
        <div class="ig-placeholder" id="storyPlaceholder">
          <span>${svgIcon('camera','Media')}</span>
    ...
    <div class="ig-tools" id="storyTools">
      <button type="button" class="ig-tool active" data-tool="media"><span>${svgIcon('camera','Media')}</span><small>Media</small></button>
      <button type="button" class="ig-tool" data-tool="text"><span>Aa</span><small>Texto</small></button>
      <button type="button" class="ig-tool" data-tool="sticker"><span>${svgIcon('sticker','Stickers')}</span><small>Stickers</small></button>
      <button type="button" class="ig-tool" data-tool="music"><span>${svgIcon('music','Música')}</span><small>Música</small></button>
      <button type="button" class="ig-tool" data-tool="effect"><span>${svgIcon('sparkle','Efectos')}</span><small>Efectos</small></button>
    </div>
...
function openStoryPanel(tool){
  ...
  if(tool==='media'){
    ...
    panel.innerHTML = `
      <div class="ig-panel-inner">
        ...
        <div class="ig-align-row">
          <button type="button" data-align="left" aria-label="Izquierda">${svgIcon('text','Izquierda')}</button>
          <button type="button" data-align="center" class="active" aria-label="Centro">${svgIcon('text','Centro')}</button>
          <button type="button" data-align="right" aria-label="Derecha">${svgIcon('text','Derecha')}</button>
        </div>
        <div class="ig-row-tools">
          <label class="tiny">Tamaño <input type="range" id="storyTextSize" min="16" max="56" value="28"></label>
          <input type="color" id="storyTextColor" value="#ffffff" title="Color">
        </div>
        <button type="button" class="ig-chip primary" id="addStoryText">Añadir texto (arrastrable)</button>
        <div id="layerInspector"><p class="muted tiny">Arrastra el texto en el canvas. Doble toque para editar.</p></div>
      </div>`;
    ...
    $('#addStoryText').onclick=()=>{
      ...
      const canvas=$('#storyTextLayer')||$('#storyCanvas');
      addStudioText(canvas, t, {
        size: Number($('#storyTextSize')?.value||28),
        color: $('#storyTextColor')?.value||'#fff',
        x: 12, y: 35
      });
      toast('Texto añadido · arrástralo y redimensiónalo');
    };
  } else if(tool==='sticker'){
    panel.innerHTML = `
      <div class="ig-panel-inner stickers-grid">
        <button type="button" class="sticker-item" data-st="location">${svgIcon('pin','Ubicación')} Ubicación</button>
        <button type="button" class="sticker-item" data-st="mention">${svgIcon('at','Mención')} Mención</button>
        <button type="button" class="sticker-item" data-st="music">${svgIcon('music','Música')} Música</button>
        <button type="button" class="sticker-item" data-st="gif">${svgIcon('gif','GIF')} GIF</button>
        <button type="button" class="sticker-item" data-st="poll">${svgIcon('poll','Encuesta')} Encuesta</button>
        <button type="button" class="sticker-item" data-st="question">${svgIcon('question','Pregunta')} Pregunta</button>
        <button type="button" class="sticker-item" data-st="hashtag">${svgIcon('hash','Hashtag')} Hashtag</button>
        <button type="button" class="sticker-item" data-st="countdown">${svgIcon('clock','Cuenta')} Cuenta regresiva</button>
        <button type="button" class="sticker-item" data-st="emoji">${svgIcon('sparkle','Sticker')} Sticker</button>
      </div>`;
    $$('.sticker-item').forEach(b=>b.onclick=()=>{
      if(b.dataset.st==='gif') openGiphyPicker(url=>addStoryGif(url), { hostEl: panel, title: 'GIF para Story' });
      else addStorySticker(b.dataset.st);
    });
  } else if(tool==='music'){
    panel.innerHTML = `<div class="ig-panel-inner">${musicPicker('storyMusicSelect')}<div id="layerInspector"><p class="muted tiny">La badge de música se puede arrastrar por el canvas.</p></div></div>`;
    bindMusicPickers(panel);
    panel.addEventListener('sekai:music-selected',e=>{
      ...
      if(track && canvas){
        addStudioMusicBadge(canvas, track);
        // lyrics overlay also draggable
        let overlay=$('#storyMusicOverlay');
        if(overlay) overlay.remove();
        const lyr=$('#storyMusicSelect_lyricsText')?.value||'';
        overlay=document.createElement('div');
        overlay.id='storyMusicOverlay';
        overlay.className='story-lyrics-overlay studio-layer';
        overlay.dataset.layer='lyrics';
        overlay.dataset.style=$('#storyMusicSelect_lyricStyle')?.value||'clean';
        overlay.innerHTML=`<small>${esc(track.title)} · ${esc(track.artist)}</small><div class="lyrics-sync-lines"></div>`;
        renderSyncedLyrics(overlay.querySelector('.lyrics-sync-lines'),lyr,0,$('#storyMusicSelect_lyricStyle')?.value||'clean');
        canvas.appendChild(overlay);
        makeStudioLayer(overlay,{kind:'lyrics',x:10,y:62,resizable:true});
      }
    });
...
function addStorySticker(type){
  if(type==='gif'){ openGiphyPicker(url=>addStoryGif(url), { hostEl: $('#storyPanel'), title: 'GIF para Story' }); return; }
  const layer=$('#storyStickersLayer')||$('#storyCanvas'); if(!layer)return;
  const labels={location:'Aquí',mention:'@amigo',music:'Now Playing',gif:'GIF',poll:'Sí / No',question:'Pregúntame',hashtag:'#Sekai',countdown:'24h',emoji:'Brillo'};
  const el=document.createElement('div');
  el.className='ig-sticker studio-layer';
  el.dataset.layer='sticker';
  el.textContent=labels[type]||type;
  layer.appendChild(el);
  makeStudioLayer(el,{kind:'sticker',x:20+Math.random()*40,y:20+Math.random()*35});
  if(window._storyState){
    window._storyState.stickers = window._storyState.stickers || [];
...
/* ========== SHORT EDITOR (Reels / videos cortos verticales) ========== */
function showShortEditor(){
  studioFullscreen(`
  <div class="cap-editor short-mode reel-mode">
    <header class="cap-top">
      <button type="button" class="cap-back" id="shortBack" aria-label="Volver">${svgIcon('close','Volver')}</button>
      <span>Short · Reel</span>
      <button type="button" class="cap-export" id="shortPublish">Publicar</button>
    </header>
    <div class="cap-preview-wrap reel-preview-wrap">
      <div class="cap-preview short-preview reel-preview" id="shortPreview">
        <div class="reel-placeholder" id="shortPlaceholder">
          ${svgIcon('video','Video')}
          <p>Video vertical (Reels)</p>
          <small>Toca para elegir un clip de hasta ~60s</small>
        </div>
        <video id="shortVideo" playsinline loop muted hidden></video>
        <div class="cap-text-overlay" id="shortTextOverlay"></div>
      </div>
    </div>
    <div class="cap-toolbar">
      <button type="button" class="cap-tool active" data-cap="media"><span>${svgIcon('video','Video')}</span><small>Video</small></button>
      <button type="button" class="cap-tool" data-cap="text"><span>Aa</span><small>Texto</small></button>
      <button type="button" class="cap-tool" data-cap="music"><span>${svgIcon('music','Música')}</span><small>Música</small></button>
      <button type="button" class="cap-tool" data-cap="fx"><span>${svgIcon('sparkle','Efectos')}</span><small>FX</small></button>
      <button type="button" class="cap-tool" data-cap="gif"><span>GIF</span><small>GIF</small></button>
    </div>
    <div class="cap-panel" id="shortPanel"></div>
    <input type="file" id="shortFile" accept="video/*" hidden>
  </div>`);
  $('#shortBack').onclick=()=>{closeStudio(); showCreatePost();};
  initShortEditor();
}
function initShortEditor(){
  const st={file:null,url:null,text:'', font:'Jakarta', size:28, align:'center', musicId:'', fx:'none', gifUrl:null};
  window._shortState=st;
  const v=$('#shortVideo');
  const render=()=>{
    const ov=$('#shortTextOverlay'); if(!ov)return;
    if(!ov.classList.contains('studio-layer')){ ov.classList.add('studio-layer'); ov.dataset.layer='texto'; makeStudioLayer(ov,{kind:'texto',x:10,y:70,resizable:true}); }
    ov.textContent=st.text||'';
  ...
  const setVideo=(file)=>{
    if(!file)return;
    st.file=file; st.url=URL.createObjectURL(file);
    if(v){ v.src=st.url; v.hidden=false; v.muted=true; v.playsInline=true; v.loop=true; v.play().catch(()=>{}); }
    $('#shortPlaceholder')?.remove();
  };
  $('#shortPlaceholder')?.addEventListener('click',()=>$('#shortFile').click());
  $('#shortPreview')?.addEventListener('click',e=>{
    if(e.target.closest('.cap-text-overlay'))return;
    if(!st.file) $('#shortFile').click();
  });
  $('#shortFile').onchange=e=>{ const f=e.target.files?.[0]; if(f) setVideo(f); };

  $$('.cap-tool').forEach(b=>b.onclick=()=>{
    $$('.cap-tool').forEach(x=>x.classList.remove('active')); b.classList.add('active');
    const p=$('#shortPanel');
    if(b.dataset.cap==='media'){
      p.innerHTML=`<button type="button" class="ig-chip" id="pickShortVid">Elegir video vertical</button>
        <p class="muted tiny">Formato tipo Reels / TikTok: vertical 9:16. Se publica como short.</p>`;
      $('#pickShortVid').onclick=()=>$('#shortFile').click();
    } else if(b.dataset.cap==='text'){
      p.innerHTML=`<textarea id="shortText" maxlength="220" placeholder="Texto sobre el video…" rows="3">${esc(st.text)}</textarea>
        <label class="tiny">Tamaño <input id="shortSize" type="range" min="16" max="48" value="${st.size}"></label>`;
      ...
      p.innerHTML=musicPicker('shortMusic')+`<div id="layerInspector"><p class="muted tiny">Arrastra la badge de música en el preview.</p></div>`;
      bindMusicPickers(p);
      p.addEventListener('sekai:music-selected',e=>{
        st.musicId=e.detail.track?.id||'';
        if(e.detail.track) addStudioMusicBadge($('#shortPreview'), e.detail.track);
      });
      ...
      p.innerHTML=`<div class="cap-fx-row">
        ...
        <button type="button" data-fx="warm">Warm</button>
      </div>`;
      $$('[data-fx]').forEach(x=>x.onclick=()=>{st.fx=x.dataset.fx;render(); if(v) v.dataset.filter=st.fx;});
    } else if(b.dataset.cap==='gif'){
      openGiphyPicker(url=>{
        st.gifUrl=url;
        const prev=$('#shortPreview');
        if(prev){
          const el=document.createElement('div');
          el.className='ig-sticker ig-sticker-gif studio-layer';
          el.dataset.layer='gif';
          el.innerHTML=`<img src="${esc(url)}" alt="" draggable="false">`;
          prev.appendChild(el);
          makeStudioLayer(el,{kind:'gif',x:20,y:20});
        }
        toast('GIF añadido · arrástralo');
      }, { hostEl: p, title: 'GIF para Short' });
    }
  });
  $$('.cap-tool')[0].click();
  ...
  $('#shortPublish').onclick=async()=>{
    const sb=requireSupabase(); if(!sb||!state.user)return;
    if(!st.file) return toast('Elige un video para el Short (tipo Reels).','error');
    const btn=$('#shortPublish'); btn.disabled=true; btn.textContent='…';
    try{
      const ext=(st.file.name.split('.').pop()||'mp4').toLowerCase();
      const path=`${state.user.id}/shorts/${crypto.randomUUID()}.${ext}`;
      ...
      if(up.error)throw up.error;
      const media_url=sb.storage.from(MEDIA_BUCKET).getPublicUrl(path).data.publicUrl;
      const m=resolvePrismTrack(st.musicId);
      const payload={
        user_id:state.user.id, type:'short', media_url,
        title:'', caption:st.text.trim(), visibility:'public', duration_seconds:null,
        music_id:m?.id||null, music_title:m?.title||null, music_artist:m?.artist||null, music_url:m?musicUrl(m):null,
        editor_data:{font:st.font,size:st.size,align:st.align,fx:st.fx,gifUrl:st.gifUrl||null,format:'reel'}
      };
...
function showImageEditor(kind){
  ...
  <div class="cap-editor image-mode">
    ...
    <div class="cap-toolbar">
      <button type="button" class="cap-tool active" data-cap="media"><span>${svgIcon('image','Media')}</span>Media</button>
      <button type="button" class="cap-tool" data-cap="filter"><span>${svgIcon('palette','Estilo')}</span>Filtro</button>
      ...
      <button type="button" class="cap-tool" data-cap="gif"><span>GIF</span>GIF</button>
    </div>
...
function initImageEditor(kind){
  ...
  $$('.cap-tool').forEach(b=>b.onclick=()=>{
    ...
    if(b.dataset.cap==='media'){
      ...
      openGiphyPicker(url=>{
        st.file=null; st.url=url; st.fromGif=true; render();
        toast('GIF seleccionado');
      }, { hostEl: p, title: 'GIF para Post' });
    }
  });
  $$('.cap-tool')[0].click();
  $('#imagePublish').onclick=async()=>{
    const sb=requireSupabase(); if(!sb||!state.user)return;
    if(!st.file && !st.url)return toast('Selecciona una imagen o GIF.','error');
    ...
    try{
      ...
      if(!url) throw new Error('Sin media');
      const {error}=await sb.from('posts').insert({
        user_id:state.user.id, type:kind==='art'?'art':(st.fromGif?'gif':'post'), media_url:url,
        title:st.title.trim(), caption:st.caption.trim(), visibility:st.visibility,
        editor_data:{filter:st.filter, frame:st.frame, fromGif:!!st.fromGif}
      });
      if(error)throw error;
...
function showVideoEditor(){
  ...
  <div class="cap-editor video-mode">
    ...
    <div class="cap-toolbar">
      <button type="button" class="cap-tool active" data-cap="media"><span>${svgIcon('video','Video')}</span>Media</button>
      <button type="button" class="cap-tool" data-cap="trim"><span>${svgIcon('scissors','Recorte')}</span>Recorte</button>
      <button type="button" class="cap-tool" data-cap="speed"><span>${svgIcon('clock','Velocidad')}</span>Velocidad</button>
      <button type="button" class="cap-tool" data-cap="filter"><span>${svgIcon('palette','Estilo')}</span>Filtro</button>
      <button type="button" class="cap-tool" data-cap="music"><span>${svgIcon('music','Música')}</span>Música</button>
...
function initVideoEditor(){
  ...
  $$('.cap-tool').forEach(b=>b.onclick=()=>{
    ...
    if(b.dataset.cap==='media'){
      ...
      p.innerHTML=musicPicker('videoMusic')+`<div id="layerInspector"><p class="muted tiny">Arrastra la badge de música.</p></div>`;
      bindMusicPickers(p);
      p.addEventListener('sekai:music-selected',e=>{
        st.musicId=e.detail.track?.id||'';
        if(e.detail.track) addStudioMusicBadge($('#vidPreview'), e.detail.track);
      });
      $('#videoMusic')?.addEventListener('change',e=>st.musicId=e.target.value);
    } else if(b.dataset.cap==='text'){
      p.innerHTML=`<textarea id="vidOverlayText" placeholder="Texto sobre el video..." rows="2"></textarea>
        <div class="ig-row-tools">
          <label class="tiny">Tamaño <input type="range" id="vidTextSize" min="16" max="56" value="28"></label>
          <input type="color" id="vidTextColor" value="#ffffff">
        </div>
        <button type="button" class="ig-chip primary" id="addVidText">Añadir texto (arrastrable)</button>
        <div id="layerInspector"></div>`;
      $('#addVidText')?.addEventListener('click',()=>{
        const t=$('#vidOverlayText')?.value?.trim(); if(!t) return toast('Escribe un texto');
        st.caption=t;
        addStudioText($('#vidPreview'), t, {
          size: Number($('#vidTextSize')?.value||28),
          color: $('#vidTextColor')?.value||'#fff',
          x: 12, y: 40
        });
        toast('Texto añadido · arrástralo');
      });
    } else if(b.dataset.cap==='gif'){
      openGiphyPicker(url=>{
        st.coverGif=url;
        const prev=$('#vidPreview');
        const el=document.createElement('div');
        el.className='ig-sticker ig-sticker-gif studio-layer';
        el.dataset.layer='gif';
        el.innerHTML=`<img src="${esc(url)}" alt="" draggable="false">`;
        prev?.appendChild(el);
        makeStudioLayer(el,{kind:'gif',x:15,y:15});
        toast('GIF añadido · arrástralo');
      }, { hostEl: p, title: 'GIF para Video' });
    }
  });
  $$('.cap-tool')[0].click();
  $('#videoPublish').onclick=async()=>{
    ...
    try{
      ...
      const {error}=await sb.from('posts').insert({
        user_id:state.user.id, type:'video', media_url:url, thumbnail_url:st.coverGif||null,
        title:st.title.trim(), caption:st.caption.trim(), visibility:st.visibility,
        duration_seconds:dur,
        music_id:m?.id||null, music_title:m?.title||null, music_artist:m?.artist||null, music_url:m?musicUrl(m):null,
        editor_data:{speed:st.speed, filter:st.filter, trimStart:st.start, trimEnd:st.end, coverGif:st.coverGif||null}
      });
      if(error)throw error;
...
async function openConversation(id){
 ...
 applyChatBackground(conv.userId||id);
 // header actions
 ...
 if(bar){
  bar.innerHTML=conv.userId?`
    <button type="button" class="icon-btn" id="chatSearchBtn" title="Buscar">${svgIcon('search','Buscar')}</button>
    <button type="button" class="icon-btn" id="chatMuteBtn" title="Silenciar">${svgIcon('music','Silenciar')}</button>
    <button type="button" class="icon-btn" id="chatToneBtn2" title="Personalizar">${svgIcon('palette','Personalizar')}</button>
  `:'';
 ...
 if(conv.userId){
  ...
  setTimeout(()=>{
    ...
    if(form && !document.getElementById('chatImageInput')){
      ...
      if(!document.getElementById('chatGifBtn')){
        const gifBtn=document.createElement('button');
        gifBtn.type='button';
        gifBtn.id='chatGifBtn';
        gifBtn.className='chat-gif-btn';
        gifBtn.title='Enviar GIF';
        gifBtn.textContent='GIF';
        form.insertBefore(gifBtn, form.firstChild.nextSibling);
        gifBtn.onclick=()=>openGiphyPicker(url=>sendChatGif(url), { title: 'GIF para el chat' });
      }
    }
    const inp=$('#chatInput');
...
async function openSocial(initialTab='friends'){showModal(`<span class="eyebrow">SEKAI SOCIAL</span><h2>Conecta con personas</h2><div class="social-tabs sk-tabs" role="tablist"><button type="button" class="sk-tab" data-social="friends">Amigos</button><button type="button" class="sk-tab" data-social="requests">Solicitudes</button><button type="button" class="sk-tab" data-social="search">Buscar</button><button type="button" class="sk-tab" data-social="library">Biblioteca</button><button type="button" class="sk-tab" data-social="settings">Ajustes</button></div><div id="socialContent" class="social-body"></div>`);$$('[data-social]').forEach(b=>b.onclick=()=>{$$('[data-social]').forEach(x=>x.classList.toggle('active',x===b));renderSocial(b.dataset.social);});const first=document.querySelector(`[data-social="${initialTab}"]`)||document.querySelector('[data-social]');if(first){first.classList.add('active');renderSocial(first.dataset.social);}else renderSocial(initialTab);}
...
if(tab==='settings'){const p=state.profile||{};c.innerHTML=`<h3>Personalización</h3><p class="muted">Estos ajustes controlan el aspecto y las animaciones de tu Sekai.</p><div class="custom-grid"><label>Tema<input id="customTheme" type="color" value="${esc(p.theme_color||'#7068e8')}"></label><label>Acento<input id="customAccent" type="color" value="${esc(p.accent_color||'#a29cf5')}"></label><label>Animaciones<select id="customMotion"><option value="full" ${p.animation_level==='full'?'selected':''}>Completas</option><option value="reduced" ${p.animation_level==='reduced'?'selected':''}>Reducidas</option><option value="off" ${p.animation_level==='off'?'selected':''}>Desactivadas</option></select></label><label>Glass<select id="customGlass"><option value="72">Suave</option><option value="86" selected>Medio</option><option value="95">Intenso</option></select></label><label>Fondo<select id="customBg"><option value="soft">Suave</option><option value="plain">Limpio</option><option value="night">Noche</option></select></label><label>Navegación<select id="customNav"><option value="normal">Normal</option><option value="compact">Compacta</option></select></label></div><label>Fondo de perfil<input id="customProfileBg" type="color" value="${esc(localStorage.getItem('sekai_profile_bg')||'#f6f2ff')}"></label>
<label>Fuente<select id="customFont"><option value="system-ui">Sistema</option><option value="'Plus Jakarta Sans',system-ui">Jakarta</option><option value="Georgia,serif">Serif</option><option value="'Courier New',monospace">Mono</option></select></label>
<button id="saveCustomization" class="primary wide">Guardar personalización</button>
<button id="previewLiveCustom" class="secondary wide" type="button">Vista previa en vivo</button><div class="settings-row"><span>Notificaciones del navegador</span><button id="notifyBtn" class="secondary">Activar</button></div><h3>Seguridad</h3><button id="resetSession" class="secondary wide">Cerrar sesión en este dispositivo</button><button id="deleteAccount" class="danger wide">Eliminar cuenta</button>`;$('#previewLiveCustom')?.addEventListener('click',()=>{applyCustomization({theme_color:$('#customTheme').value,accent_color:$('#customAccent').value,animation_level:$('#customMotion').value,glass_intensity:normalizeGlass($('#customGlass').value),background_mode:$('#customBg').value,navigation_style:$('#customNav').value});const pb=$('#customProfileBg')?.value;if(pb)document.body.style.setProperty('--profile-bg',pb);const f=$('#customFont')?.value;if(f)document.body.style.setProperty('--ui-font',f);toast('Vista previa aplicada');});
$('#saveCustomization').onclick=async()=>{const sb=requireSupabase();if(!sb||!state.user)return;const payload={theme_color:$('#customTheme').value,accent_color:$('#customAccent').value,animation_level:$('#customMotion').value,glass_intensity:normalizeGlass($('#customGlass').value),background_mode:$('#customBg').value,navigation_style:$('#customNav').value};const profileBg=$('#customProfileBg')?.value; if(profileBg){localStorage.setItem('sekai_profile_bg',profileBg);document.body.style.setProperty('--profile-bg',profileBg);}const font=$('#customFont')?.value; if(font){localStorage.setItem('sekai_font',font);document.body.style.setProperty('--ui-font',font);}const {data,error}=await sb.from('profiles').update(payload).eq('id',state.user.id).select().single();if(error)return toast(error.message,'error');state.profile=data;localStorage.setItem('sekai_customization',JSON.stringify(payload));applyCustomization(data);toast('Personalización guardada')};$('#notifyBtn').onclick=async()=>{if('Notification' in window){const p=await Notification.requestPermission();toast(p==='granted'?'Notificaciones activadas.':'Permiso no concedido.','ok')}};$('#resetSession').onclick=async()=>{await supabase.auth.signOut();location.reload()};$('#deleteAccount').onclick=()=>toast('Por seguridad, la eliminación definitiva requiere una función backend protegida.','error');}}
...
renderConversations();$('#messageFilter').oninput=renderConversations;$('#chatBack').onclick=()=>{const w=$('.messages-wrap');if(w)w.classList.remove('chat-open');};$('#newChat').onclick=()=>state.user?openSocial():openOnboarding('landing');$('#chatInfo').onclick=()=>{const id=activeConversation?.userId; if(id) openChatTonePicker(id); else toast('Abre un chat primero');};$('#chatToneBtn')?.addEventListener('click',()=>{const id=activeConversation?.userId; if(id) openChatTonePicker(id);});
...
function openChatTonePicker(chatId){
  ...
  const chatBg = localStorage.getItem('sekai_chat_bg_'+chatId) || localStorage.getItem('sekai_chat_bg') || '';
  showModal(`<div class="settings-shell"><span class="eyebrow">CHAT</span><h2>Personalizar chat</h2>
    <p class="muted">Tono y fondo solo para esta conversación.</p>
    <h3 class="tiny-h">Tono</h3>
    <div class="tone-list">
      ${['default','soft','ringtone','alarm','silent'].map(t=>`
        <button type="button" class="tone-opt ${t===cur?'active':''}" data-tone="${t}">${t}</button>`).join('')}
    </div>
    <h3 class="tiny-h">Fondo del chat</h3>
    <div class="bg-swatches">
      ${[['','Ninguno'],['#1a1228','Noche'],['#f3eeff','Lavanda'],['#0f172a','Oscuro'],['#fff7ed','Cálido'],['gradient','Degradado']].map(([v,l])=>`
        <button type="button" class="bg-swatch ${chatBg===v?'on':''}" data-cbg="${v}" style="${v==='gradient'?'background:linear-gradient(160deg,#2a1f45,#1a1228)':(v?`background:${v}`:'background:#eee')}">${l}</button>`).join('')}
    </div>
    <button type="button" class="secondary wide" id="closeTone">Listo</button>
  </div>`);
  $$('[data-tone]').forEach(b=>b.onclick=()=>{ setChatTone(chatId, b.dataset.tone); openChatTonePicker(chatId); });
  $$('[data-cbg]').forEach(b=>b.onclick=()=>{
    const v=b.dataset.cbg;
    localStorage.setItem('sekai_chat_bg_'+chatId, v);
    applyChatBackground(chatId);
    openChatTonePicker(chatId);
  });
  $('#closeTone')?.addEventListener('click',()=>{$('#modal').hidden=true});
}
function applyChatBackground(chatId){
  const body=$('#chatBody'); if(!body) return;
  const v=localStorage.getItem('sekai_chat_bg_'+(chatId||'')) || localStorage.getItem('sekai_chat_bg') || '';
  if(!v){ body.style.background=''; return; }
  if(v==='gradient') body.style.background='linear-gradient(160deg,#2a1f45,#1a1228)';
  else body.style.background=v;
}
