...
function addStoryGif(url) {
  if (!url) return;
  const layer = $('#storyStickersLayer');
  if (!layer) return;
  const el = document.createElement('div');
  el.className = 'ig-sticker ig-sticker-gif';
 ...
 <form id="editProfileForm" class="form profile-edit-form">
  <div class="st-panel active" data-panel="profile">
   ...
   <label>URL del banner
     <div class="url-with-gif">
       <input id="epBanner" type="url" placeholder="https://..." value="${esc(p.banner_url||'')}">
       <button type="button" class="gif-pick-btn" id="epBannerGif" title="Elegir GIF">GIF</button>
     </div>
   </label>
   <label>URL del avatar
     <div class="url-with-gif">
       <input id="epAvatar" type="url" placeholder="https://..." value="${esc(p.avatar_url||'')}">
       <button type="button" class="gif-pick-btn" id="epAvatarGif" title="Elegir GIF">GIF</button>
     </div>
   </label>
  ...
  <div class="st-panel" data-panel="look" hidden>
   ...
   <div class="two-col privacy-selects">
    ...
    ${sekaiSelect('epGlass',[
      {value:'0.72',label:'Suave'},
      {value:'0.86',label:'Medio'},
      {value:'0.95',label:'Intenso'}
    ], String(p.glass_intensity??0.86), {label:'Glass'})}
   </div>
   <div class="two-col privacy-selects">
    ${sekaiSelect('epBg',[
    ...
    panel.innerHTML = `
      <div class="ig-panel-inner">
        ...
        <div class="ig-align-row">
          <button type="button" data-align="left" aria-label="Alinear a la izquierda">${svgIcon('text','Izquierda')}</button>
          <button type="button" data-align="center" class="active" aria-label="Centrar">${svgIcon('text','Centro')}</button>
          <button type="button" data-align="right" aria-label="Alinear a la derecha">${svgIcon('text','Derecha')}</button>
        </div>
...
function showShortEditor(){
  ...
  <div class="cap-editor short-mode">
    ...
    <div class="cap-toolbar">
      ...
      <button type="button" class="cap-tool" data-cap="style"><span>${svgIcon('palette','Estilo')}</span>Estilo</button>
      <button type="button" class="cap-tool" data-cap="music"><span>${svgIcon('music','Música')}</span>Música</button>
      <button type="button" class="cap-tool" data-cap="fx"><span>${svgIcon('sparkle','Efectos')}</span>Efectos</button>
...
function initShortEditor(){
  const st={text:'', font:'Jakarta', size:34, bg:'gradient', align:'center', musicId:'', fx:'none', gifUrl:null};
  ...
  $$('.cap-tool').forEach(b=>b.onclick=()=>{
    ...
    if(b.dataset.cap==='text'){
      ...
      openGiphyPicker(url=>{
        ...
        let g=$('#shortGifBg');
        if(!g){ g=document.createElement('img'); g.id='shortGifBg'; g.className='short-gif-bg'; prev.insertBefore(g, prev.firstChild); }
        g.src=url;
        toast('GIF de fondo añadido');
  ...
  $('#shortPublish').onclick=async()=>{
    ...
    const payload={user_id:state.user.id,type:'short',media_url:st.gifUrl||null,title:'',caption:st.text.trim(),visibility:'public',duration_seconds:null,
      music_id:m?.id||null,music_title:m?.title||null,music_artist:m?.artist||null,music_url:m?musicUrl(m):null,
      editor_data:{font:st.font,size:st.size,background:st.bg,align:st.align,fx:st.fx,gifUrl:st.gifUrl||null}};
  ...
  $$('.cap-tool').forEach(b=>b.onclick=()=>{
    ...
    if(b.dataset.cap==='media'){
      ...
      openGiphyPicker(url=>{
        st.coverGif=url;
        let g=$('#vidGifCover');
        if(!g){ g=document.createElement('img'); g.id='vidGifCover'; g.className='vid-gif-cover'; $('#vidPreview').appendChild(g); }
        g.src=url;
        toast('GIF de portada añadido');
